import csv
import sys
#TODO CHECK IF FILE IS EMPTY OR IF FILE IS NOT CSV
    ...
    with open(first_csv, 'r') as first_file:
        with open(second_csv, 'r') as second_file:
            ...
            try:
                ...
                print('ERROR! Column given to join on does not exist', file=sys.stderr)
                return None
...
def inner_join(first_csv: str, second_csv: str, column_name: str):
    header = generate_header(first_csv, second_csv, column_name)
...
def left_join(left_csv: str, right_csv: str, column_name: str):
    header = generate_header(left_csv, right_csv, column_name)
    if header is not None:
...
def join(first_csv: str, second_csv: str, column_name: str, join_type='inner'):
    if join_type == 'inner':
        new_csv = inner_join(first_csv, second_csv, column_name)
        if new_csv is not None:
            print(new_csv)
    elif join_type == 'left':
        new_csv = left_join(first_csv, second_csv, column_name)
        if new_csv is not None:
            print(new_csv)
    elif join_type == 'right':
        new_csv = right_join(first_csv, second_csv, column_name)
        if new_csv is not None:
            print(new_csv)
    else:
